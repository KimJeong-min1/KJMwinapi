...
namespace jm
{
	StageEight::StageEight()
	{
	}
	StageEight::~StageEight()
	{
	}
	void StageEight::Initialize()
	{
		...
		graphcis::Texture* bg = Resources::Find<graphcis::Texture>(L"BG08");
		bgsr->SetTexture(bg);

		Scene::Initialize();
	}
	void StageEight::Update()
	{
		Scene::Update();
	}
	void StageEight::LateUpdate()
	{
		Scene::LateUpdate();
		...
		{
			SceneManager::LoadScene(L"StageFive");
		}
	...
	void StageEight::Render(HDC hdc)
	{
		Scene::Render(hdc);
	...
	void StageEight::OnEnter()
	{
	}
	void StageEight::OnExit()
	{
	}
}