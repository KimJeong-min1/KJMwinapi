...
namespace jm
{
	...
	void StageSix::Initialize()
	{
		...
		graphcis::Texture* bg = Resources::Find<graphcis::Texture>(L"BG06");
		bgsr->SetTexture(bg);

		Scene::Initialize();
	}
	void StageSix::Update()
	{
		Scene::Update();
	}
	void StageSix::LateUpdate()
	{
		Scene::LateUpdate();
	...
	void StageSix::OnEnter()
	{
	}
	void StageSix::OnExit()
	{
	}
}