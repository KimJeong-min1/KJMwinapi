...
namespace jm
{
	...
	void StageSeven::Initialize()
	{
		...
		graphcis::Texture* bg = Resources::Find<graphcis::Texture>(L"BG07");
		bgsr->SetTexture(bg);

		Scene::Initialize();
	}
	void StageSeven::Update()
	{
		Scene::Update();
	}
	void StageSeven::LateUpdate()
	{
		Scene::LateUpdate();
	...
	void StageSeven::Render(HDC hdc)
	{
		Scene::Render(hdc);
	...
	void StageSeven::OnEnter()
	{
	}
	void StageSeven::OnExit()
	{
	}
}